...
Class ListingController extends Controller
{
    ...
    public function listing() 
    {
        ...
        if ($s_category == '4') {
            ...
            */
            ...
            // }
                ...
                $result = $result->groupby('vendor_listing.vl_id')->distinct()->orderBy('vendor_listing.l_prime','DESC')->inRandomOrder()->get();
            ...
            $join->on('category_type_consultancy.id', '=', 'vendor_listing.l_sub_category')
                ...
                $result = $result->where('activations.completed_at','!=', null)
                ->whereNull('users.deleted_at')
                ->groupBy('vendor_listing.vl_id')
                ->orderBy('vendor_listing.l_prime','DESC')
                ->inRandomOrder()->get();   
        ...
        // })
                ...
                $result = $result->where('activations.completed_at','!=', null)->whereNull('users.deleted_at')->groupBy('vendor_listing.vl_id')
                ->orderBy('vendor_listing.l_prime','DESC')->inRandomOrder()->get();
...
//vendor_listing on product.v_id = vendor_listing.u_id where product_name="sofa set" and url_name="ShreeKrishnaFurniture"
    ...
    {   
        ...
        $detail = DB::table('product')->select('product.*','vendor_listing.u_id','vendor_listing.url_name','vendor_listing.product_category as v_product_category')
       ...
       // echo $product_id[0]->v_id; exit;
        ...
        $related = DB::table('product')->select('product.*','vendor_listing.u_id','vendor_listing.url_name','vendor_listing.product_category as v_product_category')
         ->join('vendor_listing','vendor_listing.u_id','product.v_id')
                    ->where('product.product_category',$detail[0]->product_category)
                   //->where('v_id',357)
