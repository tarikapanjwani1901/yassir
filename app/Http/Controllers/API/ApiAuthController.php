...
class ApiAuthController extends Controller
{
    ...
    public function login(Request $request)
    {
		...
		if(empty($checkUser)){
				return \Illuminate\Support\Facades\Response::json(array('success' => false,'code'    => 442,'message' => "Sorry invalid mobile number.",
			 'data'    => array()));
	...
	public function userRegistrationVerification(Request $request) {
        ...
        ->where('otp', $request->otp)->where('mobile', $request->mobile)->get()->toArray();
		if(empty($getOtp)){
				 return \Illuminate\Support\Facades\Response::json(array('success' => false,'code'    => 442,'message' => "Sorry invalid otp.",
			 'data'    => array()));
		}
		if(!empty($getOtp)){
				$currentDate = date("Y-m-d H:i:s"); 
		...
		return \Illuminate\Support\Facades\Response::json(array(
			...
			'data'    => array('user_id'=>$id,'otp'=>$request->otp,'user_type'=>$request->user_type,'first_name'=>$request->first_name,'last_name'=>$request->last_name,'mobile'=>$request->mobile)));
		}
		}
