...
class FileController extends JoshController {
    ...
    {
         $city_info = DB::table('pincode')
            ->orderBy('City_Name')
            ->groupby('City_Id')
            ->distinct()
            ->paginate(50);
        return view('admin/manage_cities',compact('city_info'));
    }
    ...
    {
        DB::table('pincode')->where('City_Id', $id)->delete();
        return 'success';
    }
    ...
    {
        DB::table('pincode')->where('City_Id', $id)->update(['status' => 'active']);
        return redirect()->back()->with('status', 'Successfully status updated'); 
    }

    public function inactive_city(Request $request,$id)
    {
       DB::table('pincode')->where('City_Id', $id)->update(['status' => 'in-active']);
       return redirect()->back()->with('status', 'Successfully status updated');
    }
}
