...
class VendorListing extends Model
{
    ...
    public static function getAllListing($cate='',$subCate='',$vendor='',$listing='')
    {
    	$allListing = DB::table('vendor_listing')
    		->select(DB::raw('ROUND(SUM(vendor_listing_review.l_review) / COUNT(vendor_listing_review.l_id)) as result,vendor_listing.*,users.*,user_details.*'))
    ...
    public static function updateListing()
    {
        ...
        if (isset($_FILES['video']['tmp_name']) && $_FILES['video']['tmp_name'] != '') {
            $file=trim(str_replace(" ","_", trim(str_replace(" ","_", $_FILES['video']['name']))));
            $uploadDirectory = public_path().'/images/' . $_POST['vl_id'].'/video';
