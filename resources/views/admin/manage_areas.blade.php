...
<div class="col-md-12 manage-clt">
    <form action="area" method="get" autocomplete="off
    ">
        ...
        <div class="input-group">
            ...
            <span class="input-group-prepend">
               ...
               <a href="{{url('/admin/managesareas')}}" data-toggle="tooltip" title="reset"><i class="fa fa-refresh" aria-hidden="true"></i></a>
            </span>
            ...
            <div class="portlet-body">
                ...
                <div class="table-responsive">
                        ...
                        <thead>
                         ...
                         <tbody>
                            @php $i = ($area_info->currentpage()-1)*$area_info->perpage() + 1 @endphp
                           @if(count($area_info))
                                
                                @foreach($area_info as $s)
                                    <tr id="tr_{{$s->City_Id}}">
                                        <td>{{$i++}}</td>
                                        <td>{{$s->Pincode}}</td>
                                        <td>{{$s->City_Name}}</td>
...
<script>
    ...
    jQuery("#btn_ok_1").on('click',function(){
        ...
        $.ajax({
           type:'POST',
           url:'managesareas/delete/'+reviewID,
    ...
    $(".inactive").on('click',function(){
        ...
        $.ajax({
           type:'POST',
           url:'managesareas/inactive_city/'+ciid,
           data:'_token = <?php echo csrf_token() ?>',
           success:function(){
    ...
    $(".active").on('click',function(){
        ...
        $.ajax({
           type:'POST',
           url:'managesareas/active_city/'+ciidss,
           data:'_token = <?php echo csrf_token() ?>',
           success:function(){
                 $('.successs').text('akljsbdkajsbdjkasd');
           }
